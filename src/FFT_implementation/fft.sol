...
contract fft {
    ...
    uint256 n;
    uint256 thetaT = (PI / N);
    int256 public phiT_real = Trigonometry.cos(thetaT);
    int256 public phiT_img = -Trigonometry.sin(thetaT);
    int256 T_real;
    int256 T_img;
    ...
    function fuck_chuck() public {
        while (k > 1) {
            n = k;
            k >>= 1;
            int256 phiT_real__temp = phiT_real;
            phiT_real = ((phiT_real * phiT_real / 1e18) - (phiT_img * phiT_img / 1e18));
            ...
            for(uint256 l = 0; l < k;l++) {
                for(uint256 a = l;a < N; a+=n ) {
                    ...
                    real_part[b] = ((T_real * t_real / 1e18) - (T_img * t_img / 1e18));
                    complex_part[b] = ((T_real * t_img / 1e18) + (T_img * t_real / 1e18));
                }
                int256 T_real_temp = T_real;
                T_real = ((T_real * phiT_real / 1e18) - (T_img * phiT_img / 1e18));
                T_img = ((T_real_temp * phiT_img / 1e18) + (T_img * phiT_real / 1e18));
            }
        }
